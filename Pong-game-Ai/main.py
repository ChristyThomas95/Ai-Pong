...
class Ping_Pong:
    ...
    def train_timmy(self, genome1, genome2, config, draw=False):
...
def eval_genomes(genomes, config):
    ...
    for i, (genome_id1, genome1) in enumerate(genomes):
        ...
        for genome_id2, genome2 in genomes[min(i+1, len(genomes) - 1):]:
            ...
            pong = Ping_Pong(win, width, height)

            force_quit = pong.train_timmy(genome1, genome2, config, draw=True)
            if force_quit:
                quit()
